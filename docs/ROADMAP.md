...
- Album discovery producers: library-gap fill and net-new album discovery (build on the v1.0.0 substrate; the release-radar new-release producer has shipped)
...
- Album-level discovery substrate shipped in v1.0.0: albums are a first-class recommendation unit (`kind` discriminator on recommendations, `album_blocks` forever-block layer, album scoring modifier, `addAlbum` single-album Lidarr approval, kind filter + Albums nav on Discover, full i18n across 15 locales). The release-radar new-release producer now populates it with `kind='album'` recommendations for new releases from tracked artists; library-gap fill and net-new album discovery producers roll out next.
