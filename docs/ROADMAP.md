...
- Discovery-only installs (no Lidarr) derive a genre profile from native listening-source metadata and local caches. The unreleased #403 follow-up adds Plex/Jellyfin/Emby/Discogs mappings plus a bounded background MusicBrainz warmer with optional Last.fm fallback; library installs keep their library genre reference.
