...
describe('createSyncOrchestrator', () => {
  ...
  it('fails the sync without replacing the snapshot when album listing fails', async () => {
    const a = sourceWithAlbums(
      'lidarr',
      [{ sourceArtistId: '1', name: 'Radiohead', mbid: 'a74b1b7f-71a5-4011-9441-d0b5e4122711' }],
      {
    ...
    expect(summary.results[0]).toMatchObject({ status: 'failed', error: 'album boom' })
    expect(store.replaceLibrarySnapshot).not.toHaveBeenCalled()
    expect(store.replaceLibraryArtists).not.toHaveBeenCalled()
    expect(recorder.fail).toHaveBeenCalledWith(1, 'album boom')
    expect(store.upsertLibrarySyncState).toHaveBeenLastCalledWith(1, 'lidarr', {
      lastSyncStatus: 'failed',
      lastSyncError: 'album boom',
    })
  })

  it('keeps the snapshot intact after mixed album listing results', async () => {
