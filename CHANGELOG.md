...
- Existing local accounts can link an SSO identity from Settings > Account after confirming the current password. Linking preserves the account's role and data, requires the same active browser session, and rejects identities already linked elsewhere. [#657](https://github.com/iuliandita/digarr/issues/657).
...
- **Logging out or switching accounts no longer leaves the previous user's data on screen.** The query cache is a module-level singleton, so a new session reused the old account's cached settings and recommendations until a browser refresh. It is now cleared on logout, on a 401, and after login, and the clear is broadcast to any other open tabs, which previously kept the old account's data until a 401 or a refocus. Reported in [#648](https://github.com/iuliandita/digarr/issues/648).
...
- **Large Lidarr libraries no longer fail to sync with an unexplained "The operation was aborted."** Lidarr returns the whole artist list in one response and offers no pagination, so a large library can take well over a minute to serialize -- one report measured ~54 MB and ~83 seconds for 7100 artists. Digarr allowed 10 seconds and then retried three times, so the sync died after ~43 seconds having asked Lidarr to serialize the entire library four times over. The full-library fetch now gets 120 seconds and no retries, tunable with `DIGARR_LIDARR_TIMEOUT_SECONDS`; every other Lidarr call keeps its short timeout so an unreachable Lidarr still fails fast. Timeouts now report the budget that elapsed instead of a bare abort, and the library fetch names the environment variable to raise. Separately, approving an album no longer downloads the entire artist list just to check whether the artist is already tracked -- it asks Lidarr for that one artist -- which is why approvals could fail on large libraries even when the scan was not running. Reported in [#589](https://github.com/iuliandita/digarr/issues/589).
- **A discovery mode whose streaming connection has gone stale now says so, instead of claiming you never connected.** Spotify, Deezer, and TIDAL discovery modes reported every failure -- an expired token that could not be refreshed, an encryption-key mismatch, a database error -- as "Connect <provider> to use this mode.", even while Settings still showed the provider as connected. Those cases now say the connection is no longer usable and to reconnect, and the underlying error is logged instead of being discarded. A genuinely missing connection still reads as before.
- **TIDAL token refresh now authenticates the same way the initial connect does.** The authorization-code exchange sends HTTP Basic credentials while the refresh sent them in the form body. Only one of the two can be right for a given TIDAL app, and a mismatch would have surfaced as an unexplained disconnect roughly an hour after connecting. Refresh now mirrors the exchange. Relevant to the unvalidated TIDAL flow tracked in [#553](https://github.com/iuliandita/digarr/issues/553).
- **Spotify and Deezer connect failures now report the same detail TIDAL already did.** A token endpoint that could not be reached, or that answered without an `access_token`, previously surfaced as a generic failure (or, for an unreachable Spotify endpoint, a server error page). All three providers now map to the same `oauth_error` stages, and the upstream error body is logged secret-redacted.
- **Credential redaction now catches `client_secret=` and `refresh_token=`.** The redactor keyed on a word boundary, which snake_cased parameter names slip past, so those two could reach logs unredacted.
...
- **Provider OAuth connect flows are hardened, and starting one no longer risks your existing connection.** In-flight Spotify, Deezer, and TIDAL authorizations moved out of the stored-token row into their own table, so abandoning a re-connect leaves the working connection untouched (previously it was replaced by a pending marker and the provider went silently disconnected). The one-time `state` is now stored only as a SHA-256 digest, its 10-minute expiry is enforced instead of merely recorded, and the row is deleted the moment the state is redeemed, so an abandoned or failed flow can no longer leave a redeemable state behind. Starting a connect also sets a short-lived `HttpOnly` transaction cookie, mirroring the OIDC login flow: the callback must arrive from the browser that started it, which closes an account-injection path where someone holding a leaked `state` could bind their own streaming account to another user's Digarr account. Two new `oauth_error` stages report the new refusals (`state_expired`, `browser_mismatch`), `POST /api/v1/auth/oauth/:provider/initiate` is rate limited to 5/min, and OAuth token-refresh failures no longer log the upstream response body unredacted. Note for TIDAL: with `ALLOWED_ORIGIN` unset, the browser-supplied callback URI is now accepted only outside production and only for a loopback host -- production installs must set `ALLOWED_ORIGIN` to connect TIDAL. Any authorization that was in flight during the upgrade needs to be restarted; completed connections are unaffected.
...
- **Last.fm fallback for popular-album approval.** When you approve an artist by its most popular albums, Digarr ranks them by Spotify popularity; if Spotify is not connected for your account it now falls back to Last.fm global playcounts (when a Last.fm API key is configured) instead of giving up. The two scales are never blended -- the first source that returns albums wins -- and the approve options now gate on which popularity source is actually reachable for you.
...
- **Database-migration errors now use a single `application/problem+json` shape.** The `POST /api/v1/admin/migrate-backend` endpoint returns the `MigrationReport` body only on `200`; every error (including a `422` verification failure, which now carries the report under a `report` field) uses the standard problem envelope `{ type, title, status, code }`. This removes the previous ambiguity where a `422` could carry two different body shapes.
- **Default Docker image is now the alpine variant.** The unsuffixed tags (`:latest`, `:stable`, `:vX.Y.Z`) now ship on the Alpine base instead of Debian slim, which removes unused OS packages (perl, ncurses, and others) and the bulk of their CVEs -- the runtime image drops from 13 HIGH/CRITICAL OS-package advisories to 2. The image is smaller and behaves identically (Bun on musl, PGlite is WASM, the healthcheck shells out to `bun`). Upgrading in place is transparent -- all durable state lives in volumes, the PGlite on-disk format is identical across bases, and the container UID is unchanged (`bun:bun` 1000:1000), so a debian `:latest` deployment can pull the new alpine `:latest` and restart with no data migration. Deployments that need the Debian/glibc base -- e.g. those that exec glibc tooling inside the container -- can pin the new `-debian` suffixed tags (e.g. `:1.10.0-debian`, `:debian`).

### Fixed

- **Approving an artist with selected albums now actually monitors and downloads those albums.** When you approved an artist but picked only specific albums, Digarr added the artist to Lidarr unmonitored and then tried to monitor the chosen albums immediately -- but Lidarr populates an artist's album list asynchronously, so that lookup usually ran before the albums existed and silently monitored nothing. The album never got grabbed, and the approval still reported success. Digarr now polls Lidarr until the selected albums appear, monitors them, and triggers an album search so they download. If an album genuinely never shows up, the artist is still added and its Lidarr link is kept -- so a retry reconciles against the existing artist instead of creating a duplicate -- and the approve toast warns that the album could not be monitored yet. The job record also captures the monitor mode and selected album IDs so the case is debuggable from the job list.
- **Lidarr root folder is resolved dynamically instead of assuming id 1.** Adding an artist to Lidarr previously sent a hardcoded root-folder id of 1, which failed on instances whose root folder had a different id. Digarr now resolves the first available root folder from the Lidarr API.
- **Setup wizard no longer 400s when AI fields are left blank, and webhook credentials are redacted in logs.** Completing setup without filling in the optional AI provider fields previously returned a 400 because null values failed validation; those fields are now correctly optional. Separately, webhook URLs and tokens are now redacted in application logs so notification credentials are not written to disk in plaintext.
