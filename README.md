...
> [!NOTE]
> **v1.15.1 is out.** A bugfix release. Large Lidarr libraries no longer die mid-sync with an unexplained "The operation was aborted." -- Lidarr returns the whole artist list in one unpaginated response, which can take well over a minute, and Digarr was allowing ten seconds and then retrying three times. The full-library fetch now gets 120 seconds and no retries (tunable with `DIGARR_LIDARR_TIMEOUT_SECONDS`), and approving an album asks Lidarr for one artist instead of downloading the entire library. A discovery mode whose streaming connection has gone stale now says so instead of claiming you never connected, TIDAL token refresh authenticates the same way the initial connect does, Spotify and Deezer connect failures report the same detail TIDAL already did, and a failed login over plain HTTP names the setting to change. The release also moves Hono past four published advisories. The v1.15.0 features it builds on -- Spotify Followed Artists and TIDAL Favorite Artists discovery modes, hardened provider OAuth, encrypted webhook URLs, and bulk reconciliation review -- are unchanged. See the [latest release notes](https://github.com/iuliandita/digarr/releases/latest) and [CHANGELOG.md](CHANGELOG.md) for details. If you run into something, [open an issue](https://github.com/iuliandita/digarr/issues).
>
