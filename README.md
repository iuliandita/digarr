...
> [!NOTE]
> **v1.15.0 is out.** Digarr adds two discovery modes built on your own collections: Spotify Followed Artists (needs the `user-follow-read` scope, so reconnect Spotify once to grant it) and TIDAL Favorite Artists, the latter shipped experimental because its OAuth flow has not yet run against a live TIDAL account. The Spotify top-artists genre signal now spans all three listening windows. The release also hardens provider OAuth: in-flight authorizations moved into their own table with a hashed, single-use, expiry-enforced state and a browser-bound transaction cookie, so starting a re-connect no longer disturbs a working connection. Webhook notification URLs are encrypted at rest and masked in the settings API, connect successes and failures now show a message on the Settings page instead of only a query string, library reconciliation review explains why each row is unresolved and supports atomic bulk ignore, and all 22 tables moved from `serial` to `GENERATED BY DEFAULT AS IDENTITY` primary keys. See the [latest release notes](https://github.com/iuliandita/digarr/releases/latest) and [CHANGELOG.md](CHANGELOG.md) for details. If you run into something, [open an issue](https://github.com/iuliandita/digarr/issues).
>
