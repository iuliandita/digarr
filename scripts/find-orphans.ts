...
const body = files.filter(f => !f.includes('i18n/messages/')).map(f => readFileSync(f, 'utf8')).join('\n')
...
function usesDynamic(prefix: string) {
  return body.includes('`' + prefix) || body.includes("'" + prefix + "${") || new RegExp('\\b' + prefix.replace('.', '\\.') + '\\$\\{').test(body)
}
